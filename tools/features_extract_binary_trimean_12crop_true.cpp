...
int main(int argc, char** argv) {
  ...
  */
  ...
  LOG(INFO) << caffe_test_net->blob_by_name("data")->width(); //TODO
	...
	///crop_id == batch_size ?
	int num_of_crop = 12;
  vector<int> height_offset_list;
  vector<int> width_offset_list;
  vector<int> is_mirror_list;
  height_offset_list.push_back(0);                                      width_offset_list.push_back(0);                                       is_mirror_list.push_back(0);
  height_offset_list.push_back(0);                                      width_offset_list.push_back(image_size - image_crop_size + 1);        is_mirror_list.push_back(0);
  height_offset_list.push_back(image_size - image_crop_size + 1);       width_offset_list.push_back(0);                                       is_mirror_list.push_back(0);
  height_offset_list.push_back(image_size - image_crop_size + 1);       width_offset_list.push_back(image_size - image_crop_size + 1);        is_mirror_list.push_back(0);
  height_offset_list.push_back((image_size - image_crop_size + 1) / 2); width_offset_list.push_back((image_size - image_crop_size + 1) / 2);  is_mirror_list.push_back(0);
  height_offset_list.push_back(-1);                                     width_offset_list.push_back(-1);                                      is_mirror_list.push_back(0);
  height_offset_list.push_back(0);                                      width_offset_list.push_back(0);                                       is_mirror_list.push_back(2);
  height_offset_list.push_back(0);                                      width_offset_list.push_back(image_size - image_crop_size + 1);        is_mirror_list.push_back(2);
  height_offset_list.push_back(image_size - image_crop_size + 1);       width_offset_list.push_back(0);                                       is_mirror_list.push_back(2);
  height_offset_list.push_back(image_size - image_crop_size + 1);       width_offset_list.push_back(image_size - image_crop_size + 1);        is_mirror_list.push_back(2);
  height_offset_list.push_back((image_size - image_crop_size + 1) / 2); width_offset_list.push_back((image_size - image_crop_size + 1) / 2);  is_mirror_list.push_back(2);
  height_offset_list.push_back(-1);                                     width_offset_list.push_back(-1);                                      is_mirror_list.push_back(2);
  ...
  for (int frame_id = 0; frame_id < frame_files.size(); ++frame_id) {
    ...
    for (int crop_id = 0; crop_id < num_of_crop; crop_id++) {
			height_offset = height_offset_list[crop_id];
      width_offset = width_offset_list[crop_id];
      int is_mirror = is_mirror_list[crop_id];
      if (height_offset == -1 || width_offset == -1) {
        height_offset = width_offset = 0;
        cv::resize(cv_img_orig, cv_img, cv::Size(image_crop_size, image_crop_size));
      } else {
        cv::resize(cv_img_orig, cv_img, cv::Size(image_size, image_size));
      }
      int mirror_value = is_mirror? image_crop_size - 1:0;
			...
			for (int i_channel = 0; i_channel < n_channel; ++i_channel) {
				for (int height = 0; height < image_crop_size; ++height) {
					for (int width = 0; width < image_crop_size; ++width) {
						int insertion_index = crop_id * n_image_crop_pixel + (i_channel * image_crop_size + height) * image_crop_size + mirror_value + (1 - is_mirror) * width;
